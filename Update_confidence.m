function [ tmp_P ] = Update_confidence( tmp_A,tmp_D1,X,maxL,per_D_size,label_data ,threshold,o,tmp_P,n)
%Update_confidence 置信度矩阵更新
%ini_D1 字典矩阵
%threshold 阈值
% sizeM = size(tmp_P(size(label_data,2)+1:size(X,2),i));
% unlabel_sample_num = 300;
% index_unlabel_data = randperm(sizeM(2),unlabel_sample_num);
% unlabel_data=pre_data_no_noise(:,index_unlabel_data);
